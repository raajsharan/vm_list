...
def create_app() -> Flask:
    ...
    def dashboard():
        ...
        # VM-creation trend aggregated by month over the last 12 months
        # (VMware created_date field, across all ESXi/vCenter sources).
        import calendar as _cal
        import datetime as _dt
        created_by_date = database.get_vm_created_by_date()
        end             = _dt.date.today()
        month_tot: dict = {}
        for ds, cnt in created_by_date.items():
            month_tot[str(ds)[:7]] = month_tot.get(str(ds)[:7], 0) + cnt   # 'YYYY-MM'
        seq, yy, mm = [], end.year, end.month
        for _ in range(12):
            seq.append((yy, mm))
            mm -= 1
            if mm == 0:
        ...
        seq.reverse()
        monthly_series = [
            {"label": _cal.month_abbr[m], "count": month_tot.get(f"{y:04d}-{m:02d}", 0)}
            for (y, m) in seq
        ]
        monthly_total = sum(p["count"] for p in monthly_series)
        ...
        return render_template(
            ...
            recent_notifs=database.recent_notifications(limit=6),
        )
    ...
    def asset_details():
        ...
        # MAC lookup mapping — enriches IPs for VMs that VMware has no IP for
        ...
        for vm in raw_vms:
            ...
            # Resolve MAC-mapped IPs from uploaded mapping file
            macs = vm.get("macs", [])
            ...
            # Build ordered, deduplicated display IP list (mapped first, then VMware)
            seen: set = set()
            display_ips: list = []
            ...
            # Asset status check: use _check_asset_ips so both mapped + VMware are tried
            asset_status_label = (
            ...
            # Best single IP for the form value (mapped preferred)
            first_ip  = display_ips[0]["ip"] if display_ips else ""
